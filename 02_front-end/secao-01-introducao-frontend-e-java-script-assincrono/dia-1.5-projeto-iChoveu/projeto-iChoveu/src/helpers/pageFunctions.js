import { searchCities, getWeatherByCity, getWeatherSevenDays } from './weatherAPI';
...
export function createCityElement(cityInfo) {
  ...
  botao.addEventListener('click', async () => {
    const newObject = await getWeatherSevenDays(url);
    showForecast(newObject);
  });
...
export async function handleSearch(event) {
  ...
  const resultCidades = await searchCities(searchValue);

  const resultado = await Promise.all(resultCidades
    .map((cidade) => getWeatherByCity(cidade.url)));

  resultado.map((cidade) => ulElement.appendChild(createCityElement(cidade)));
